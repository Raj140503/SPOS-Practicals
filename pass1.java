    ...
    public void generateOutput(String[][] asm,String[][] mot)
    {
        ...
        for(int i = 0;i<asm.length;i++)
        {
            for(int j = 0;j<4;j++)
            {
                ...
                if(asm[i][j].matches("[a-zA-Z]") && !asm[i][j].equals("LTORG"))
                {
                ...
                if (asm[i][j].equals("LTORG")) {
                    while (literalPoolIndex < lit_index) {
                        String poolStr = "(" + literalPoolIndex + ")";
                        pol.get(i).add(poolStr);
                        literalPoolIndex++;
                    }
                }
            }
